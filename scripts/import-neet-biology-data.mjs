 ...
 * 
 * Usage: node scripts/import-neet-biology-data.mjs
 *        or: npm run import:neet-biology
 * 
 * This script:
...
async function getOrCreateTopic(
  ...
  // First, check if topic exists by name
  ...
  if (topic) {
    // Topic exists - update order number and status if needed
    let needsUpdate = false;
    if (topic.orderNumber !== topicOrderNumber) {
      // Check if another topic already has this orderNumber
      const existingTopicWithOrder = await Topic.findOne({
        chapterId: chapterId,
        orderNumber: topicOrderNumber,
        _id: { $ne: topic._id }, // Exclude current topic
      });
      
      if (existingTopicWithOrder) {
        // Another topic has this orderNumber - find next available
        const maxOrder = await Topic.findOne({ chapterId })
          .sort({ orderNumber: -1 })
          .select("orderNumber");
        topic.orderNumber = maxOrder?.orderNumber ? maxOrder.orderNumber + 1 : topicOrderNumber;
        console.log(
          `      ⚠️  Order ${topicOrderNumber} taken, using ${topic.orderNumber} for topic: ${topicName}`
        );
      } else {
  ...
  // Topic doesn't exist - check if orderNumber is already taken
  const existingTopicWithOrder = await Topic.findOne({
    chapterId: chapterId,
    orderNumber: topicOrderNumber,
  });
  ...
  if (existingTopicWithOrder) {
    // Order number is taken - find next available
    const maxOrder = await Topic.findOne({ chapterId })
      .sort({ orderNumber: -1 })
      .select("orderNumber");
    topicOrderNumber = maxOrder?.orderNumber ? maxOrder.orderNumber + 1 : topicOrderNumber;
    console.log(
      `      ⚠️  Order number conflict, using ${topicOrderNumber} for topic: ${topicName}`
    );
  }
  ...
  try {
    ...
    if (error.code === 11000) {
      // Duplicate key error - topic with same chapterId and orderNumber exists
      console.log(
        `      ⚠️  Duplicate detected, finding existing topic: ${topicName}`
      );
      // Find the existing topic and return it
      topic = await Topic.findOne({
        chapterId: chapterId,
        orderNumber: topicOrderNumber,
      });
      if (!topic) {
        // If still not found, find by name as fallback
        topic = await Topic.findOne({
          name: topicName,
          chapterId: chapterId,
        });
      ...
      if (topic) {
        console.log(`      ℹ️  Using existing topic: ${topicName}`);
      }
    } else {
      throw error;
    }
